...
{
	int i1, i2, coordIdx;
	float8 bestFactor = -1.0, threshold = ctx->pointThreshold;
	int bestCoord = 0;

	/*
	 * Select best coordinate to line sweep. We would like to use coordinate
	 * where two datasets are most far from each other. Use ratio of difference
	 * of coordinate averages to standard deviation along this coordinate as
	 * selection criteria.
	 */
	for (coordIdx = 0; coordIdx < 3; coordIdx++)
	{
		float8 avg1 = 0.0, avg2 = 0.0, stddev = 0.0, factor;
	...
	for (i1 = 0; i1 < count1; i1++)
	{
		...
		/*
		 * Iterate over second dataset until it more than threshold above
		 * to current point of first dataset.
		 */
		j = i2;
		while (j < count2 &&
									   ...
									   Box3DInfo *boxes2, int count2)
{
	...
	int bestCoord = 0, threshold = ctx->box3dThreshold;

	/*
	 * Select best coordinate to line sweep. We would like to use coordinate
	 * where two datasets are most far from each other. Use ratio of difference
	 * of 3D box projection center averages to square average of 3D box
	 * projection size as selection criteria.
	 */
	for (coordIdx = 0; coordIdx < 3; coordIdx++)
	{
		float8 avg1 = 0.0, avg2 = 0.0, avgsize = 0.0, factor;
	...
	for (i1 = 0; i1 < count1; i1++)
	{
		 ...
		 * than threshold above to current upper bound of the first dataset
		 * 3D box.
		 * */
		j = i2;
		while (j < count2 &&
